...
from .wordsim import (
    get_crosslingual_wordsim_scores,
...
class Evaluator(object):
    def __init__(self, trainer):
        ...
        self.disp_calculation = getattr(trainer.params, "more_exp", False)
    ...
    def monolingual_wordsim(self, to_log):
        ...
        if src_ws_scores is not None:
            ...
            logger.info(
                "Monolingual source word similarity score average: %.5f"
                % src_ws_monolingual_scores
            )
        ...
        if tgt_ws_scores is not None:
            ...
            logger.info(
                "Monolingual target word similarity score average: %.5f"
        ...
        if src_ws_scores is not None and tgt_ws_scores is not None:
            ws_monolingual_scores = (
                src_ws_monolingual_scores + tgt_ws_monolingual_scores
            ) / 2
            logger.info(
                "Monolingual word similarity score average: %.5f"
                % ws_monolingual_scores
            )
    ...
    def monolingual_wordanalogy(self, to_log):
        ...
        if src_analogy_scores is not None:
            ...
            logger.info(
                "Monolingual source word analogy score average: %.5f"
                % src_analogy_monolingual_scores
            )
        ...
        if self.params.tgt_lang and tgt_analogy_scores is not None:
            ...
            logger.info(
                "Monolingual target word analogy score average: %.5f"
                % tgt_analogy_monolingual_scores
            )
    ...
    def crosslingual_wordsim(self, to_log):
        ...
        logger.info(
            "Cross-lingual word similarity score average: %.5f" % ws_crosslingual_scores
        )
    ...
    def word_translation(self, to_log):
        ...
        for method in ["nn", "csls_knn_100"]:
            results = get_word_translation_accuracy(
                ...
                more_exp=self.disp_calculation,
            )
    ...
    def dist_mean_cosine(self, to_log):
        ...
        for dico_method in ["nn", "csls_knn_100"]:
            ...
            dico = build_dictionary(
                src_emb, tgt_emb, _params, s2t_candidates, t2s_candidates
            )
            # mean cosine
            if dico is None:
                mean_cosine = -1e9
            ...
            mean_cosine = (
                mean_cosine.item()
                if isinstance(mean_cosine, torch_tensor)
                else mean_cosine
            )
            logger.info(
    ...
    def eval_dis(self, to_log):
        ...
        logger.info(
            "Discriminator source / target predictions: %.5f / %.5f"
            % (src_pred, tgt_pred)
        )
        ...
        dis_accu = (
            src_accu * self.src_emb.num_embeddings
            + tgt_accu * self.tgt_emb.num_embeddings
        ) / (self.src_emb.num_embeddings + self.tgt_emb.num_embeddings)
        logger.info(
            "Discriminator source / target / global accuracy: %.5f / %.5f / %.5f"
