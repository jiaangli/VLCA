...
def generate_polysemy_dicts(table, pairs, name, num_cal):
    num_meaning = {}
    ...
    for filename, condition in files:
        with open(Path("./data/dicts/poly") / f"{name}{filename}.txt", 'w') as file_writer:
            ...
            file_writer.close()
        num_cal[filename].append(len(open(Path('./data/dicts/poly') / f'{name}{filename}.txt', 'r').readlines()))
...
if __name__ == "__main__":
    ...
    for dict_path in original_path.iterdir():
        ...
        generate_polysemy_dicts(words_info, pairs, dict_path.name[:-4], num_cal)
