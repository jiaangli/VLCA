...
class DispersionCalculator:
    ...
    def get_sorted_dispersion(self):
        if self.model_type == "VM":
            ...
            indices = [
                np.where(unique_indices == i)[0] for i in range(len(unique_words))
            ]
        ...
        concepts_dis_sorted = sorted(
            concepts_dis_dict.items(), key=lambda kv: (kv[1], kv[0])
        )
...
if __name__ == "__main__":
    ...
    model_type, dim = (
        MODEL_CONFIGS[model_name].model_type,
        MODEL_CONFIGS[model_name].dim,
    )
    if model_type == "LM":
        root_path = (
            f"{RunConfig.common.alias_emb_dir}/LM/{model_name}_{dim}_per_object.pth"
        )
    else:
        root_path = (
            f"{RunConfig.common.alias_emb_dir}/dispersion/{model_name}_images_embs"
        )
