...
def get_dict(
    ...
    for seed in seeds:
        ...
        ratios = (
            [0.1, 0.5, 1, 5, 10, 15, 20, 25, 30, 40, 50, 60, 70] if ratio_flag else [70]
        )
        ...
        for ratio in ratios:
            ...
            write_data_to_file(
                train, save_root / f"train_{seed}_{file_suffix}{ratio_mark}.txt"
            )
            write_data_to_file(
                test, save_root / f"test_{seed}_{file_suffix}{ratio_mark}.txt"
            )
...
if __name__ == "__main__":
    ...
    ratio_flag = True
    save_root = (
        Path("./data/dicts/ratio_exps")
        if ratio_flag
        else Path("./data/dicts/imagenet/base")
    )
